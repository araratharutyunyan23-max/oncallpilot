...
def run_ingest(corpus_dir: Path | None = None) -> dict:
    ...
    try:
        for doc in docs:
            ...
            # One transaction per document: the content_sha write and its chunks
            # commit together or not at all. connect() is autocommit, so without
            # this a failure after upsert (embed/insert) would leave the doc row
            # carrying the final sha with zero chunks — permanently "unchanged"
            # and never re-indexed. transaction() works under autocommit too.
